...
export const getPinnedMessageId = (telegramId: string): number | undefined => {
  try {
    ...
    console.error(
      `[DB] Error getting pinned message id for ${telegramId}:`,
      error
    );
    return undefined;
  }
...
export const setPinnedMessageId = (
  ...
  try {
    db.prepare(
      'UPDATE users SET pinned_message_id = ? WHERE telegram_id = ?'
    ).run(messageId, telegramId);
  } catch (error) {
    console.error(
      `[DB] Error setting pinned message id for ${telegramId}:`,
      error
    );
  }
};
...
export const getPinnedMessageUpdatedAt = (
  ...
  try {
    const row = db
      .prepare(
        'SELECT pinned_message_updated_at FROM users WHERE telegram_id = ?'
      )
      .get(telegramId) as { pinned_message_updated_at?: number } | undefined;
    return row?.pinned_message_updated_at;
  } catch (error) {
    console.error(
      `[DB] Error getting pinned message updated_at for ${telegramId}:`,
      error
    );
    return undefined;
  }
...
export const setPinnedMessageUpdatedAt = (
  ...
  try {
    db.prepare(
      'UPDATE users SET pinned_message_updated_at = ? WHERE telegram_id = ?'
    ).run(timestamp, telegramId);
  } catch (error) {
    console.error(
      `[DB] Error setting pinned message updated_at for ${telegramId}:`,
      error
    );
  }
};
