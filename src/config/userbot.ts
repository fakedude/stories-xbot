  ...
  public static async reset(): Promise<void> {
    if (Userbot.client) {
      try {
        ...
        console.error('[Userbot] Error while disconnecting:', error);
      }
    }
    Userbot.client = null;
...
async function initClient() {
  ...
  await client.start({
    ...
    password: async () => {
      if (!password)
        throw new Error('USERBOT_PASSWORD is required for this account!');
      return password;
    },
    phoneCode: async (_isCodeViaApp?: boolean) => {
      if (!phoneCode)
        throw new Error('USERBOT_PHONE_CODE is required for first login!');
      return phoneCode;
    },
