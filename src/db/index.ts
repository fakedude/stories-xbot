...
`);
const blockedColumns = db
  .prepare('PRAGMA table_info(blocked_users)')
  .all() as any[];
...
export function getNextQueueItem(): DownloadQueueItem | null {
  ...
  if (row?.task_details) {
    ...
    return {
      ...
      task, // Use the fully preserved task object.
      status: row.status,
...
export function getQueueStats(jobId: number): {
  position: number;
  eta: number;
} {
  const job = db
...
export function listMonitors(telegram_id: string): MonitorRow[] {
  return db
    .prepare(`SELECT * FROM monitors WHERE telegram_id = ?`)
    .all(telegram_id) as MonitorRow[];
}
...
export function getMonitor(id: number): MonitorRow | undefined {
  return db.prepare(`SELECT * FROM monitors WHERE id = ?`).get(id) as
    | MonitorRow
    | undefined;
}
...
export function countMonitors(telegram_id: string): number {
  const row = db
    .prepare(`SELECT COUNT(*) as c FROM monitors WHERE telegram_id = ?`)
