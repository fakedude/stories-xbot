...
export const markProcessingFx = createEffect((jobId: string) =>
  db.markProcessing(jobId)
);
...
export const wasRecentlyDownloadedFx = createEffect(
  async (params: {
    ...
    return db.wasRecentlyDownloaded(
      params.telegram_id,
      params.target_username,
      params.hours
    );
...
export const getDownloadCooldownRemainingFx = createEffect(
  (params: { telegram_id: string; target_username: string; hours: number }) =>
    db.getDownloadCooldownRemaining(
      params.telegram_id,
      params.target_username,
      params.hours
    )
...
export const findPendingJobFx = createEffect((telegram_id: string) =>
  db.findPendingJobId(telegram_id)
);

export const getQueueStatsFx = createEffect((jobId: number) =>
  db.getQueueStats(jobId)
);

// Fetch recent history of downloads for admin reporting
export const getRecentHistoryFx = createEffect((limit: number) =>
...
export const wasProfileRequestedRecentlyFx = createEffect(
  (params: { telegram_id: string; target_username: string; hours: number }) =>
    db.wasProfileRequestedRecently(
      params.telegram_id,
      params.target_username,
      params.hours
    )
