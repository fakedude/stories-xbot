...
import { isValidStoryLink, sendTemporaryMessage, timeout } from 'lib';
import { Api } from 'telegram';
import { FloodWaitError } from 'telegram/errors';
import { NotifyAdminParams, UserInfo } from 'types';
...
export const getAllStoriesFx = createEffect(async (task: UserInfo) => {
  try {
    await sendTemporaryMessage(bot, task.chatId, '⏳ Fetching story lists...');
    ...
    const activeStories: Api.TypeStoryItem[] =
      activeResult.stories?.stories || [];
    ...
    if (activeStories.length > 0 && pinnedStories.length > 0) {
      pinnedStories = pinnedStories.filter(
        (p) => !activeStories.some((a) => a.id === p.id)
      );
    }

    console.log(
      `[GetStories] Initial fetch for ${task.link}: ${activeStories.length} active, ${pinnedStories.length} initial pinned.`
    );

    if (!task.nextStoriesIds) {
      let lastPinnedStoryId: number | null =
        pinnedStories.length > 0
          ? pinnedStories[pinnedStories.length - 1].id
          : null;
      ...
      while (lastPinnedStoryId !== null) {
        ...
        if (olderPinnedResult && olderPinnedResult.stories.length > 0) {
          const newPinnedStories = olderPinnedResult.stories.filter(
            (newStory) =>
              !activeStories.some((aS) => aS.id === newStory.id) &&
              !pinnedStories.some((pS) => pS.id === newStory.id)
          );
          if (newPinnedStories.length > 0) {
            pinnedStories.push(...newPinnedStories);
            lastPinnedStoryId =
              newPinnedStories[newPinnedStories.length - 1].id;
        ...
        if (fetchedCountInLoop > 500 && isDevEnv) {
          console.warn(
            '[GetStories] DEV MODE: Safety break in pagination loop.'
          );
          break;
        }
      }
      console.log(
        `[GetStories] Total pinned stories after pagination for ${task.link}: ${pinnedStories.length}`
      );
    }
    ...
    console.error(
      `[GetStories] Error in getAllStoriesFx for task ${task.link}:`,
      error
    );
    if (
      ...
      console.warn(
        '[GetStories] Possible session error, reinitializing Userbot'
      );
      try {
        await Userbot.reset();
        await Userbot.getInstance();
      } catch (error_) {
        console.error('[GetStories] Failed to reinitialize Userbot:', error_);
      }
    }
...
export const getParticularStoryFx = createEffect(async (task: UserInfo) => {
  try {
    ...
    const match = /^(?:https?:\/\/)?t\.me\/([^\/]+)\/s\/(\d+)/i.exec(
      task.link.trim()
    );
    if (!match) {
    ...
    console.log(
      `[GetStories] Fetching particular story for ${usernameOrChannelId}, story ID: ${storyId}`
    );
    ...
    console.error(
      `[GetStories] ERROR in getParticularStoryFx for ${task.link}:`,
      error
    );
    if (error instanceof FloodWaitError) {
      const seconds = error.seconds || 60;
