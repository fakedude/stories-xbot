...
import { chunkMediafiles, sendTemporaryMessage, timeout } from 'lib';
...
import {
  ...
  StoriesModel,
  UserInfo,
} from 'types'; // <--- Added MappedStoryItem for explicit typing
...
// =========================================================================
export async function sendPinnedStories({
  stories,
  task,
}: SendStoriesArgs): Promise<void> {
  try {
    ...
    if (!isPrivileged && mapped.length > STORY_LIMIT_FOR_FREE_USERS) {
      console.log(
        `[SendPinnedStories] Limiting non-premium user ${task.chatId} to ${STORY_LIMIT_FOR_FREE_USERS} stories.`
      );
      wasLimited = true;
    ...
    if (storiesWithoutMedia.length > 0) {
      ...
      try {
        // Added try/catch for this section if not already present
        ...
        console.error(
          `[SendPinnedStories] Error re-fetching stories without media: ${error}`
        );
    ...
    console.log(
      `[SendPinnedStories] [${task.link}] Preparing to download ${mapped.length} pinned stories.`
    );
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(
        () => reject(new Error('Download process timed out after 5 minutes.')),
        300000
      )
    );
    ...
    console.log(
      `[SendPinnedStories] [${task.link}] downloadStories function completed.`
    );
    ...
    console.log(
      `[SendPinnedStories] [${task.link}] Found ${uploadableStories.length} uploadable pinned stories after download.`
    );
    ...
    if (uploadableStories.length > 0) {
      ...
      for (const [i, album] of chunkedList.entries()) {
    ...
    console.log(
      `[SendPinnedStories] [${task.link}] Processing finished successfully.`
    );
  } catch (error: any) {
    // <--- Explicitly typed error as any
    notifyAdmin({
      status: 'error',
      task,
    ...
    console.error(
      `[SendPinnedStories] [${task.link}] CRITICAL error occurred:`,
      error
    );
    try {
      await bot.telegram.sendMessage(
        task.chatId,
        ' An error occurred while processing pinned stories. The admin has been notified.'
      );
    } catch (error_) {
      /* ignore */
    }
    ...
    console.log(
      `[SendPinnedStories] [${task.link}] Function execution complete.`
    );
  }
}
