...
import { MappedStoryItem, StoriesModel } from 'types'; // Ensure: yarn add p-limit (if not already)

// --- Configuration for Concurrency ---
// If you get FLOOD_WAIT errors from Telegram, lower this.
const DOWNLOAD_CONCURRENCY_LIMIT = 3;
const limit = pLimit(DOWNLOAD_CONCURRENCY_LIMIT); // <--- This import is correct
...
export async function downloadStories(
  ...
  console.log(
    `[DownloadStories] Starting download of ${stories.length} ${storiesType} stories. Concurrency: ${DOWNLOAD_CONCURRENCY_LIMIT}.`
  );

  const downloadPromises = stories.map((storyItem: MappedStoryItem) =>
    limit(async () => {
      if (signal?.aborted) return;
      const mediaExists = Boolean(storyItem.media);
      const isNoforwards = Boolean(storyItem.noforwards);

      // If media doesn't exist, skip (do not skip just for noforwards)
      if (!mediaExists) {
        console.log(
          `[DownloadStories] Story ${storyItem.id} (${storiesType}): Skipping, media missing.`
        );
        return;
      }
      ...
      if (isNoforwards) {
        console.log(
          `[DownloadStories] Note: Attempting to download story ${storyItem.id} marked 'noforwards'.`
        );
      }

      try {
        console.log(
          `[DownloadStories] Attempting download for story ID ${storyItem.id} (${storiesType})`
        );
        ...
        if (buffer instanceof Buffer && buffer.length > 0) {
          ...
          storyItem.bufferSize = Number.parseFloat(
            (buffer.byteLength / (1024 * 1024)).toFixed(2)
          );
          console.log(
            `[DownloadStories] Downloaded story ID ${storyItem.id} (${storiesType}), Type: ${storyItem.mediaType}, Size: ${storyItem.bufferSize} MB.`
          );
          onProgress?.(storyItem);
        } else {
          console.log(
            `[DownloadStories] Story ID ${storyItem.id} (${storiesType}): Empty or invalid buffer.`
          );
        }
      } catch (error: any) {
        // Explicitly type error as any for now
        console.error(
          `[DownloadStories] Error downloading story ID ${storyItem.id} (${storiesType}): ${error.message}`
        );
        if (error.errorMessage?.startsWith('FLOOD_WAIT_')) {
          const waitSeconds = Number.parseInt(
            error.errorMessage.split('_').pop() || '30'
          );
          console.warn(
            `[DownloadStories] Hit FLOOD_WAIT for ${waitSeconds}s on story ${storyItem.id}.`
          );
        }
      }
  ...
  console.log(
    `[DownloadStories] Finished all download attempts for ${stories.length} ${storiesType} stories. Success: ${successfulDownloads}, Failed: ${failedDownloads}.`
  );
...
export function mapStories(stories: Api.TypeStoryItem[]): StoriesModel {
  ...
  stories.forEach((x: Api.TypeStoryItem) => {
    // Explicitly type x
    if (!x || !('id' in x)) return;
    ...
    if (
      'photo' in x.media &&
      x.media.photo &&
      typeof x.media.photo === 'object'
    ) {
      story.mediaType = 'photo';
    } else if (
      'document' in x.media &&
      x.media.document &&
      typeof x.media.document === 'object'
    ) {
    ...
    story.noforwards =
      'noforwards' in x && typeof x.noforwards === 'boolean'
        ? x.noforwards
        : false;
  ...
  console.log(
    `[MapStories] Mapped ${mappedStories.length} out of ${stories.length} initial stories.`
  );
