...
function scheduleInvoiceCheck(
  ...
  const doCheck = async () => {
    ...
    if (result.unexpectedSenders?.length) {
      if (botInstance)
    ...
    if (newInv && newInv.id !== invoice.id) {
      invoice = newInv;
      if (botInstance)
        await botInstance.telegram.sendMessage(
          userId,
...
export async function getBtcPriceUsd(): Promise<number> {
  ...
  const prices: number[] = [];
  for (const [i, res] of requests.entries()) {
    if (res.status === 'fulfilled') {
  ...
  if (prices.length === 0) throw new Error('Unable to fetch BTC price');
...
export async function checkPayment(
  ...
  if (invoice.from_address) {
    ...
    for (const tx of txs) {
      ...
      if (
        fromAddrs.includes(invoice.from_address) &&
        txid &&
        !isTxidUsed(txid)
      ) {
...
export async function verifyPaymentByTxid(
  ...
  let toUs: any;
  for (const o of outs || []) {
    const addr = o.scriptpubkey_address || o.addr || o.addresses?.[0];
    if (!addr) continue;
